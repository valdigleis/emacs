...
#+BEGIN_CENTER
#+CAPTION: ⚠️ **Atenção:**  IA instalação do zsh permite tira o maior proveito desta configuração, a instalação pode ser feita (no Debian) usando o comando =sudo apt install zsh=
#+END_CENTER
